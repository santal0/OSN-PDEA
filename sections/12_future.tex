...
This project provides an initial profile-aware auditing framework for measuring differential dark pattern exposure in mobile applications. However, as discussed in the limitations, the current framework remains partly manual, uses static profiles, and evaluates a bounded set of applications and target-budget interaction trajectories. Future work can extend the framework in several directions so that profile-conditioned dark pattern exposure can be measured more automatically, more realistically, and at a larger scale.
...
For the visual component of such a detector, transformer-based vision models provide one possible starting point because they can represent relations among interface regions, text blocks, buttons, and promotional modules. The original Vision Transformer and hierarchical variants such as Swin Transformer are relevant examples, and self-supervised pretraining could reduce the amount of manually annotated interface data needed before training a screenshot-level DPS detector \cite{dosovitskiy2021an,liu2021swin,he2022masked}. Efficiency would matter in a large audit, but architecture optimization should remain secondary to the methodological goal: producing auditable DPS labels that preserve context and can be reviewed by humans.

A practical future system could use a hybrid pipeline. First, screenshots and UI trees would be processed by OCR and visual detectors. Second, a dark pattern classifier or multimodal model would produce candidate DPS labels with confidence scores. Third, human annotators would only review uncertain or high-risk cases. This would reduce annotation cost while preserving quality control. Such an approach would make it possible to audit many more applications and interaction records than the current archived 1,403-record dataset.
...
The current experiment targets 16 applications, 3 profiles, and 30 steps per profile-app pair, corresponding to 1,440 planned observations and 10,080 planned DPS labels. The archived analyzable dataset contains 1,403 interaction records and 9,821 DPS labels because five trajectories are shorter. This scale is sufficient for a course project and demonstrates the feasibility of profile-conditioned exposure estimation. However, larger-scale auditing is necessary before making broader claims about mobile app ecosystems.
...
Mixed-effects models could account for the fact that observations are nested within apps, app categories, profiles, and app-profile trajectories. Clustered bootstrap confidence intervals could quantify uncertainty around exposure estimates without relying too heavily on parametric assumptions. Block permutation tests could resample or relabel whole trajectory blocks rather than treating adjacent steps as independent. Longitudinal analysis could examine whether exposure converges, accumulates, or changes across interaction trajectories. These methods would make the framework more statistically robust, especially when extended to larger datasets.
