...
For the visual component of such a detector, transformer-based vision models provide a natural starting point because they can represent global relations among interface regions, text blocks, buttons, and promotional modules. The original Vision Transformer showed that image patches can be processed directly with transformer architectures, while hierarchical variants such as Swin Transformer make this representation more suitable for structured visual scenes and dense recognition tasks \cite{dosovitskiy2021an,liu2021swin}. Self-supervised pretraining methods such as masked autoencoders could also reduce the amount of manually annotated interface data needed before training a screenshot-level DPS detector \cite{he2022masked}.

Efficiency would be important if the audit is scaled to many apps, profiles, and interaction steps. Efficient Vision Transformer research suggests several possible ways to reduce the computational cost of screenshot understanding, including dynamic token pruning, token reorganization, slow-fast token evolution, differentiable compression rates, token merging, local-similarity-based merging, neighbor-aware token reduction, adaptive patch sizing, and frequency-aware token reduction \cite{rao2021dynamicvit,liang2022evit,xu2022evo,chang2023making,chen2023diffrate,bolya2022token,haroun2024leveraging,li2025neighbor,choudhury2026faster,lee2026frequency}. These techniques are relevant because many mobile-interface screenshots contain repeated background regions, large blank areas, or visually redundant modules, so a future DPS detector may not need to process every patch at full cost. More broadly, surveys of efficient Vision Transformers provide useful guidance for selecting architectures that balance accuracy, latency, and deployment cost in a large-scale auditing pipeline \cite{10508091}.
