...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/pictures/experimental_scale_matrix.png}
    \caption{Experimental Scale Matrix}
    \label{fig:experimental-scale-matrix}
\end{figure}
...
For each application and each profile, the audit targets thirty interaction steps. With sixteen applications and three profiles, this planned protocol produces:
\[
...
DPS = [I, F, P, C, S, N, FA],
\]
where \(I\) denotes Interface Interference, \(F\) denotes Friction, \(P\) denotes Pressure, \(C\) denotes Contextual Deception, \(S\) denotes Sneaking, \(N\) denotes Nagging, and \(FA\) denotes Forced Action. Since each record contains seven DPS labels, the planned protocol contains:
\[
1{,}440 \times 7 = 10{,}080 \ \text{DPS labels}.
\]
The archived analyzable dataset is slightly smaller: it contains 1{,}403 interaction records and 9{,}821 DPS labels because five app-profile trajectories ended before the 30-step target. Analyses use the effective trajectory length for each app-profile pair, denoted \(T_{\mathrm{app},A}\) or \(n_{\mathrm{app},A}\), rather than assuming that every observed trajectory has exactly thirty records.

The target budget of thirty steps reflects a balance between feasibility and repeated exposure estimation. A single screenshot would be too fragile because dark pattern exposure may depend on timing, page state, recommendation flow, login status, promotion availability, or the user's previous actions within the app. Repeated interactions provide a more stable basis for estimating profile-conditioned exposure \(E(D \mid A)\). At the same time, the audit remains small enough to be manually executed and annotated within the constraints of a course project. The running mean convergence curves introduced later in the report are used to examine whether exposure estimates gradually stabilize as more interaction steps are collected. In this sense, the target interaction budget is not assumed to be perfect; instead, it is treated as an experimental design choice whose stability can be evaluated empirically.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/pictures/controlled_audit_procedure.png}
    \caption{Controlled Audit Procedure}
    \label{fig:controlled-audit-procedure}
\end{figure}
...
The experiment uses the same target number of interaction steps across all app-profile pairs. Each profile is assigned a thirty-step target in each application, so the planned comparison is not designed around one profile simply interacting more than another. Because five archived trajectories are shorter, the actual analysis uses the effective observed length \(n_{\mathrm{app},A}\) for each app-profile pair. A fixed waiting time is used before screenshots are captured, so that dynamic interface elements have time to load and the screenshot timing is more consistent. The same DPS annotation rules are applied across all records. This helps ensure that a label such as pressure, friction, or forced action is interpreted according to the same criteria regardless of the application or profile.
...
\begin{enumerate}
    ...
    \item Repeat the process until the target budget of thirty steps is completed for the current application-profile pair, or record the effective observed length \(n_{\mathrm{app},A}\) if the trajectory ends early.
\end{enumerate}
