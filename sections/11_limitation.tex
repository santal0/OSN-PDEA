...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/pictures/limitations_map.png}
    \caption{Limitations Map}
    \label{fig:limitations_map}
\end{figure}

\section{Target Interaction Budget}

The limitation is that every app-profile pair is assigned a target 30-step interaction budget, while the archived analyzable data contain five shorter trajectories. This matters because some exposure mechanisms are cumulative: recommendations, promotions, retention prompts, and permission requests may only appear after longer account histories or repeated engagement.

This report mitigates that limitation by treating 30 steps as a target audit window rather than a claim about full-life app usage. The analysis uses the effective observed length \(n_{\mathrm{app},A}\) for each app-profile trajectory, and the running mean curves show that several exposure estimates stabilize within the available 20--30 observed steps. The key value of the study is comparative measurement under a shared target protocol, not exhaustive simulation of user lifetime behavior.
...
\section{Nested Step-level Records}

The statistical analysis treats step-level records as the main observations, but those records are nested within app-profile trajectories. Consecutive steps from the same trajectory are not fully independent because they share the same app, profile configuration, account state, session history, and interaction context. This nesting means that ordinary step-level tests can understate uncertainty if they ignore within-trajectory dependence.

The current statistical results should therefore be interpreted as descriptive and association-oriented evidence for the archived audit sample. Future work should use methods that respect the trajectory structure, such as block permutation at the app-profile trajectory level, clustered bootstrap confidence intervals, or mixed-effects models with random effects for apps and trajectories. These methods would provide more reliable uncertainty estimates when the framework is scaled to larger and more balanced datasets.
