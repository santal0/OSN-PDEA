...
where \(I\) denotes Interface Interference, \(F\) denotes Friction, \(P\) denotes Pressure, \(C\) denotes Contextual Deception, \(S\) denotes Sneaking, \(N\) denotes Nagging, and \(FA\) denotes Forced Action. Each screenshot collected during the interaction trajectory is converted into one DPS vector. The vector records observable exposure for each dimension. Some dimensions are naturally binary, while others are recorded as numeric exposure scores when the annotation protocol distinguishes intensity, repeated interruption, or additional effort.
...
In the current implementation, DPS is treated as a mixed numeric exposure vector. For a given screenshot, a dimension value of 0 means that the corresponding mechanism is absent, and any value greater than 0 means that it is present. Binary dimensions are coded as 0 or 1. Intensity-oriented dimensions can use larger nonzero scores, such as an ordinal interference score, an extra-operation count for friction, or a repeated-interruption count for nagging. For occurrence-rate analysis, the project binarizes the raw score using \(D_i>0\). For intensity analysis, it averages the raw scores directly. The following subsections define each DPS dimension and provide practical annotation guidance.
...
In the current version of the DPS framework, each dimension is represented as a nonnegative numeric exposure score:

\[
d_i \geq 0,
\]

where \(d_i = 0\) means that the corresponding DPS dimension is absent in the screenshot, and \(d_i>0\) means that it is present. Binary dimensions use 0 and 1. Numeric dimensions can use larger values when the protocol records intensity, added effort, or recurrence. A complete DPS vector therefore consists of seven nonnegative scores. For example, a screenshot may be represented as:
...
The mixed numeric scale is chosen because several DPS dimensions have meaningful intensity variation. Interface Interference can vary from mild salience imbalance to a strongly dominant platform-preferred path. Friction can be represented by the additional number of operations required to refuse, cancel, or exit relative to accepting or continuing. Nagging can be represented by a repeated-interruption count within the trajectory. Pressure, Contextual Deception, Sneaking, and Forced Action are usually coded as binary indicators in the present dataset, but the framework still treats all dimensions as components of one numeric exposure vector.

This scale supports two complementary analyses. Occurrence-rate analysis first converts each raw score to \(1[D_i>0]\), so the mean is interpretable as the proportion of records in which a DPS dimension appears. Intensity analysis averages the raw numeric values, so means can exceed 1 for dimensions scored with counts or ordinal levels. Table captions and figure captions distinguish these two cases. Pairwise differences between profiles can then be computed as \(\Delta(A_i, A_j)\), and statistical validation can be conducted using permutation tests, q-values, L1 distance, and Cramér's V. Categorical association statistics such as Cramér's \(V\) use the binarized occurrence variable, while raw-score heatmaps and mean tables report intensity.
