...
\[
E(D \mid A) = \frac{1}{T}\sum_{t=1}^{T} D_t.
\]

Here, \(T\) is the number of audited interaction steps for profile \(A\), and \(D_t\) is the DPS vector observed at step \(t\). In the project design, each profile is audited through repeated controlled interactions, and the resulting vectors are averaged to estimate the typical exposure pattern associated with that profile.

Each dimension of \(E(D \mid A)\) represents the proportion of interaction steps in which the corresponding dark pattern dimension appeared for profile \(A\). For example, if the Pressure dimension of \(E(D \mid A)\) is \(0.40\), this means that pressure-related mechanisms appeared in approximately 40 percent of the audited interaction states for that profile. Similarly, a higher Forced Action value indicates that the profile more frequently encountered interface states requiring account creation, login, permission granting, or another mandatory action before continuing. This interpretation makes the exposure vector intuitive: it converts a sequence of annotated screenshots into a compact profile-level summary.
...
\[
E(D \mid A, app) = \frac{1}{T}\sum_{t=1}^{T} D_{app,A,t}.
\]

In this expression, \(D_{app,A,t}\) denotes the DPS vector observed in application \(app\), under profile \(A\), at interaction step \(t\). The value \(T\) refers to the number of interaction steps conducted for that profile within that application. This estimate allows the analysis to compare different profiles while holding the application fixed.
...
\begin{figure}[htbp]
    ...
    \caption{Representative visualization of observed \(L_1\) exposure distances across comparison scopes.}
...
This stabilization analysis supports the fixed 30-step audit design. If exposure estimates continue to fluctuate heavily after many steps, the audit budget may be insufficient. If the running means gradually stabilize, then the final estimate at \(T=30\) becomes more reliable as a summary of the audited trajectory. In this project, the observed stabilization pattern suggests that repeated interaction is necessary and that the fixed-step design provides a more robust estimate than single-state inspection. Nevertheless, stabilization should be understood as empirical support for the audit design, not as proof that all possible long-term exposure patterns have been captured.
...
The framework also uses Cramér's \(V\) to measure the strength of association between profile type and DPS exposure. In the reported analysis, Cramér's \(V\) is computed from contingency tables built on binarized DPS presence, where each annotation is converted to present versus absent before tabulation. If an ordinal 0/1/2 scoring scheme were used in a future extension, the current statistic would require collapsing the positive levels into a binary presence indicator or replacing Cramér's \(V\) with an ordinal association measure. In the present report, the binary contingency formulation keeps the statistic aligned with the annotation scheme used for the audited data.
