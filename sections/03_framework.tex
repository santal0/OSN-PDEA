...
(a_2, s_2, d_2),
\ldots,
(a_{T_{\mathrm{app},A_i}}, s_{T_{\mathrm{app},A_i}}, d_{T_{\mathrm{app},A_i}})
\big),
\label{eq:trajectory}
\end{equation}

where $a_t$ is the action performed at step $t$, $s_t$ is the resulting interface state or screenshot, $d_t$ is the DPS annotation assigned to that state, and \(T_{\mathrm{app},A_i}\) is the effective observed trajectory length for the application-profile pair. In this project, each app-profile pair is assigned a target budget of 30 steps, but the archived analysis uses the effective length when a trajectory ends early. The trajectory therefore connects three components: the action generated by the profile, the interface state returned by the application, and the dark pattern exposure observed in that state.
