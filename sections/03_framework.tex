...
The implementation layer may instantiate these categories with a finer-grained action set, such as \texttt{visit\_homepage}, \texttt{search\_keyword}, \texttt{browse\_or\_scroll\_page}, \texttt{click\_recommended\_item}, \texttt{like\_or\_favorite}, \texttt{follow\_account}, \texttt{stay\_on\_content}, \texttt{decision\_interface\_visit}, \texttt{close\_popup}, and \texttt{back}. These implementation-level actions are mapped back to the four conceptual categories before exposure analysis, so the mathematical definitions below remain unchanged while the operational traces can stay more detailed.
