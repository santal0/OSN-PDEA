...
Table~\ref{tab:profile-behavior-probabilities-appendix} summarizes the behavior probabilities for the three controlled profiles: the high-payment shopping profile, the low-payment browsing profile, and the bargain-hunter profile. The probabilities should sum to one within each profile.
...
Table~\ref{tab:dps-guideline} provides the planned structure of the full annotation guideline. The final appendix should include detailed examples from the collected screenshots where appropriate.

\begin{table}[htbp]
    \centering
    \caption{Planned DPS annotation guideline.}
    ...
    \begin{tabularx}{\textwidth}{@{}p{1.75cm}XXXX@{}}
        ...
        A visually attractive button should not be labeled as interference unless it meaningfully distorts choice visibility or salience. \\
        \addlinespace
        ...
        Additional charges should not be labeled as sneaking if they are clearly disclosed before the decision point. \\
        \addlinespace
...
The final guideline should also clarify the scoring scheme used for each dimension. For example, dimensions such as Pressure, Contextual Deception, Sneaking, and Forced Action may be treated as binary labels, while dimensions such as Interface Interference, Friction, and Nagging may allow ordinal levels if the annotation protocol distinguishes between weak and strong exposure. Any such scoring choice should be reported consistently in the final appendix.
...
Each heatmap should use the DPS dimensions as one axis and either applications, profiles, or application categories as the other axis, depending on the visualization design. These heatmaps are intended to support the main finding that different profiles show different exposure patterns. They also allow readers to identify which DPS dimensions are more prominent in particular app categories.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/statistics/heatmap_mean_exposure_all.png}
    \caption{Complete mean DPS exposure heatmap for all applications and controlled profiles.}
...
\begin{figure}[htbp]
    ...
    \caption{Detailed DPS exposure heatmap for shopping applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Detailed DPS exposure heatmap for food-delivery and eating-related applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Detailed DPS exposure heatmap for entertainment applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Detailed DPS exposure heatmap for social applications.}
    \label{fig:heatmap-social-detail}
\end{figure}

The captions in this appendix should describe what each heatmap represents without restating unsupported causal claims. For example, the text should use careful language such as ``the heatmap indicates higher observed exposure'' or ``the pattern suggests profile-associated differences in exposure.''

\section{Appendix E: Statistical Visualization Set}
\label{app:statistical-visualizations}

Appendix E includes the complete statistical visualization set currently exported by the analysis pipeline. These figures complement the main report by showing q-value patterns, permutation-test summaries, observed L1 distances, and Cramér's \(V\) association strengths. Running mean convergence curves should be added here when the corresponding figures are exported from the analysis script.
...
\input{sections/appendix_curves.tex}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/statistics/mannwhitney_qvalue_heatmap_all.png}
    \caption{Mann-Whitney U BH FDR q-value heatmap across all audited applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Mann-Whitney U BH FDR q-value heatmap for shopping applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Mann-Whitney U BH FDR q-value heatmap for food-delivery and eating-related applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Mann-Whitney U BH FDR q-value heatmap for entertainment applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Mann-Whitney U BH FDR q-value heatmap for social applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Observed \(L_1\) exposure distances by comparison scope.}
...
The full convergence curves should be interpreted as descriptive stability evidence rather than proof of causal convergence. If the running mean becomes less volatile after repeated interactions, this suggests that the fixed interaction budget is sufficient to produce a more stable estimate of profile-conditioned exposure. However, the appendix should also note that longer-term interaction histories may still reveal additional exposure dynamics.

\section{Appendix F: Statistical Validation Notes}
\label{app:statistical-validation-tables}

Appendix F documents how the statistical validation results should be interpreted. The main report and Appendix~\ref{app:statistical-visualizations} report the exported plots for p-values, q-values, L1 distance, and Cramér's \(V\). These plots should be read together rather than as independent proof from any single statistic.

The statistical validation should be presented as evidence of association rather than causation. Since the experiment compares controlled profile-conditioned interaction traces, significant differences indicate that observed exposure patterns are unlikely to be explained solely by random profile assignment under the tested conditions. They should not be interpreted as proving that the application intentionally targets a particular user group.
...
In addition to statistical significance, the appendix should report practical difference measures. The \(L_1\) distance between two profile-level DPS exposure vectors can be used to describe the magnitude of exposure difference:
\[
