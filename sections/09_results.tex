...
This section reports the empirical results of the profile-aware dark pattern auditing experiment. The audit covers 16 mobile applications and three controlled user profiles, giving 48 app-profile trajectories. The protocol targeted 30 interaction steps for each app-profile pair, corresponding to 1,440 planned observations. A verification of the archived \texttt{steps.csv} and \texttt{steps.jsonl} files shows that 43 trajectories contain the full 30 steps, while five trajectories are shorter: WangYiYun low-payment browsing has 20 steps, WeiBo bargain hunter has 22 steps, WeiBo high-payment shopping has 23 steps, ZhiHu low-payment browsing has 23 steps, and WangYiYun bargain hunter has 25 steps. The analyzable archive therefore contains 1,403 interaction records. Since each interaction record was annotated with the seven-dimensional DPS vector, the resulting dataset contains 9,821 dimension-level DPS labels. These labels provide the basis for estimating profile-conditioned dark pattern exposure, comparing exposure differences across profiles, and examining whether observed differences are likely to reflect systematic profile-dependent variation rather than random fluctuation.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        App-profile trajectories & 48 total; 43 complete 30-step runs \\
        Steps per app-profile pair & Target: 30; archived range: 20--30 \\
        Planned / archived interaction records & 1,440 / 1,403 \\
...
Overall, the results suggest that dark pattern exposure is not uniform across controlled user profiles. Different profiles produce visibly different DPS distributions, and these differences appear across multiple forms of analysis. The heatmaps show that the average raw exposure intensity varies not only across applications and app categories, but also across user profiles. The running mean convergence curves further indicate that exposure estimates gradually stabilize as interaction steps accumulate, using the effective observed length of each trajectory. Finally, the statistical validation metrics, including Mann-Whitney U q-values, permutation tests, L1 distance, and Cramér's V, provide converging evidence that profile type is associated with measurable differences in dark pattern exposure.

The clearest aggregate profile gap appears in Interface Interference. Pressure is also important, but it varies by category: the all-app and food-delivery tables do not support a claim that the bargain-hunter profile always receives stronger Pressure exposure. This does not imply that platforms deliberately single out any profile with dark patterns. A more cautious interpretation is that profile-conditioned behavior changes the interaction trajectory, and different trajectories lead users into different interface regions where dark patterns are more or less likely to appear. For example, a user who frequently enters purchase, membership, coupon, or payment interfaces naturally observes more decision-stage interface designs than a user who mainly browses information feeds. In this sense, the results support the central claim of this project: dark pattern exposure should be analyzed as a profile-conditioned distribution along interaction trajectories, rather than as a static property of an application.

\section{Finding 1: Different Profiles Show Different DPS Patterns}

\paragraph{Result.}
The three controlled profiles show different DPS exposure patterns. At the aggregate level, the high-payment shopping profile has the highest mean raw DPS intensity, followed by the bargain-hunter profile and the low-payment browsing profile. The difference is not uniform across dimensions: the clearest aggregate profile gap appears in Interface Interference, while Pressure and Forced Action vary more by application category. Contextual Deception and Sneaking remain comparatively low in the overall dataset.
...
\begin{table}[htbp]
    \centering
    \caption{Profile-level raw DPS intensity means across all audited applications.}
...
\paragraph{Evidence.}
Table~\ref{tab:profile-mean-dps} reports raw DPS intensity means computed from the archived step-level CSV files. Figure~\ref{fig:mean-exposure-all} provides the corresponding heatmap. In this heatmap, rows represent controlled profiles and columns represent the seven DPS dimensions: Interface Interference, Friction, Pressure, Contextual Deception, Sneaking, Nagging, and Forced Action. The color intensity of each cell represents the average raw exposure score for a profile-dimension combination. A darker or more saturated cell therefore indicates stronger average exposure intensity in that DPS dimension under a particular profile condition. Occurrence-rate analyses, when used, are based on the binary indicator \(D_i>0\).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/statistics/heatmap_mean_exposure_all.png}
    \caption{Raw DPS intensity mean heatmap across all audited applications and controlled profiles.}
    \label{fig:mean-exposure-all}
\end{figure}

\paragraph{Food-delivery evidence.}
The category-level result for food-delivery applications shows a clearer profile gap than the global average alone. Table~\ref{tab:eating-mean-dps} summarizes the raw DPS intensity means for the food-delivery category across all audited applications in that scope. Figure~\ref{fig:mean-exposure-eating} shows the corresponding heatmap, where the separation between profiles is especially visible in Forced Action and several other dimensions. Pressure does not follow a simple bargain-hunter-highest pattern in this category.

\begin{table}[htbp]
    \centering
    \caption{Profile-level raw DPS intensity means for food-delivery applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Raw DPS intensity mean heatmap for food-delivery and eating-related applications.}
    \label{fig:mean-exposure-eating}
\end{figure}

\paragraph{Social evidence.}
Social applications show the clearest category-level separation among the audited domains. Table~\ref{tab:social-mean-dps} summarizes the raw DPS intensity means for social applications across the audited sample. Figure~\ref{fig:mean-exposure-social} shows the corresponding heatmap, where Interface Interference, Friction, and Nagging differ visibly across profiles.

\begin{table}[htbp]
    \centering
    \caption{Profile-level raw DPS intensity means for social applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Running mean raw DPS intensity curve for WangYiYun under the high-payment shopping profile.}
...
\begin{figure}[htbp]
    ...
    \caption{Running mean raw DPS intensity curve for WangYiYun under the low-payment browsing profile.}
...
\begin{figure}[htbp]
    ...
    \caption{Running mean raw DPS intensity curve for WangYiYun under the bargain-hunter profile.}
...
\paragraph{Caveat.}
Stabilization within the audited trajectories does not mean that all possible dark pattern exposure in an application has been exhausted. Some dark patterns may appear only after longer-term use, repeated purchases, account aging, seasonal promotions, or accumulated behavioral history. In addition, the five shorter archived trajectories reduce balance across profiles and categories: the all-app permutation tests use 463 low-payment browsing records, 473 high-payment shopping records, and 467 bargain-hunter records. The convergence result therefore supports the reliability of this course-scale experiment, but it does not remove the need for longer-term, balanced, or larger-scale audits.
...
\begin{table}[htbp]
    \centering
    \caption{Statistical summary of profile-dependent DPS exposure differences based on binarized occurrence \(D_i>0\).}
...
\begin{figure}[htbp]
    ...
    \caption{Mann-Whitney U BH FDR q-value heatmap for dimension-level raw DPS intensity comparisons across all audited applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Observed L1 exposure distances by comparison scope based on binarized occurrence \(D_i>0\).}
...
\begin{figure}[htbp]
    ...
    \caption{Raw DPS intensity mean heatmap for shopping applications.}
...
\begin{figure}[htbp]
    ...
    \caption{Raw DPS intensity mean heatmap for entertainment applications.}
...
For e-commerce applications, the most prominent dimensions in the archived data are Forced Action and Pressure, with a smaller amount of Interface Interference and Nagging. Sneaking is not prominent in this category-level table. This pattern is consistent with the commercial structure of shopping interfaces. Users are frequently guided through product recommendation pages, discount panels, membership prompts, shopping carts, and checkout-related screens. In these contexts, applications may require commitment before proceeding, emphasize limited-time discounts, visually prioritize promotional options, or make alternative choices less visible. The high-payment and bargain-hunter profiles are especially likely to encounter such interfaces because their keyword and action distributions lead them toward commercial decision points. These observations align with prior studies showing that shopping environments are common sites for scarcity messages, social proof, hidden costs, and other manipulative design strategies \cite{mathur2019darkpatterns}.

Food-delivery applications show consistently high Forced Action exposure in the project results, followed by Pressure and Interface Interference. This may be associated with interface flows that require location access, login, phone verification, address selection, or permission granting before users can fully browse menus, place orders, or access coupons. Pressure and Nagging also appear in this category, often through prompts for membership, coupons, delivery discounts, push notifications, or limited-time offers. Because food-delivery apps often combine urgent consumption contexts with location-based services and promotional systems, users may encounter multiple forms of commitment-requesting interfaces. However, this should be understood as an observed pattern in the audited sample rather than a universal property of all food-delivery applications.

Content platforms show a different exposure profile. Entertainment applications are dominated by Forced Action and Interface Interference in the archived data, while social applications show the highest average Interface Interference and also meaningful Pressure, Friction, and Nagging. A user may be encouraged to sign in, enable notifications, follow accounts, remain in the feed, or continue consuming recommended content. These mechanisms may not always correspond to direct commercial payment, but they can still shape user autonomy and attention. For example, recommendation loops may keep users in a sequence of suggested content, while notification prompts may repeatedly interrupt browsing. Such patterns suggest that dark pattern exposure in content platforms is often related to engagement retention rather than immediate purchase conversion.

Across the four audited categories, Contextual Deception and Sneaking remain comparatively rare. The stronger category-level signals come from Forced Action, Interface Interference, Pressure, Friction, and Nagging. This does not mean that deceptive or hidden-choice mechanisms are absent from mobile applications in general; it means that they are not the dominant exposure patterns in the archived trajectories used in this experiment.

Taken together, the category-level results show that profile-dependent exposure interacts with app-domain characteristics. Shopping and eating applications tend to expose users to more commitment- or decision-stage pressure, while entertainment and social platforms show stronger interface interference, retention prompts, and access-control moments. These findings reinforce the need to analyze dark patterns not only as isolated interface elements, but also as exposure patterns shaped by profile, behavior, app category, and interaction trajectory.
