...
csv_path = 'results/statistics/all_steps.csv'

df = pd.read_csv(csv_path)

DPS_DIMS = ["I", "F", "P", "C", "S", "N", "FA"]

output_dir = 'figures/curves'
...
for pid in profiles:
    for app in apps:
        ...
        plt.title(f"Running E(D_i | A={pid}) - [{app.upper()}]", fontsize=16, fontweight='bold')
        plt.xlabel("Step", fontsize=14, fontweight='bold')
        plt.ylabel("Running mean exposure", fontsize=14, fontweight='bold')
        plt.legend(bbox_to_anchor=(1.05, 0.5), loc='center left')
        plt.tight_layout()
        
        filename = f"curve_E_{pid}_{app}.png"
