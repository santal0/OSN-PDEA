...
file_path = 'results/statistics/all_steps.csv'
...
for i, prof in enumerate(profiles):
    ...
    axes[i].set_title(f"{prof} Profile", fontsize=24, fontweight='bold', pad=15)
    axes[i].set_xlabel("E-Commerce Platforms", fontsize=20, fontweight='bold', labelpad=10)
    axes[i].tick_params(axis='x', labelsize=12)
    axes[i].tick_params(axis='y', labelsize=13)

axes[0].set_ylabel("DPS Dimensions", fontsize=20, fontweight='bold', labelpad=10)

mappable = axes[0].collections[0]
cbar_ax = fig.add_axes([0.93, 0.15, 0.02, 0.7])
cbar = fig.colorbar(mappable, cax=cbar_ax)
cbar.set_label('Dark Pattern Exposure Rate', fontsize=18, labelpad=10)
...
output_name = 'figures/heatmap.png'
